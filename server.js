...
const connectionString =
  ...
  "postgresql://time_track_db_qwet_user:2EINugVdgXJDL1EBIO21Yyr5wjBhJ7Pr@dpg-cvtnbda4d50c73akebt0-a.oregon-postgres.render.com/time_track_db_qwet";
...
pool.query("SELECT NOW()", (err, res) => {
  if (err) {
    ...
    console.log(
      "เชื่อมต่อฐานข้อมูลสำเร็จ เวลาปัจจุบันของ server:",
      res.rows[0].now
    );
    initializeDatabase();
  }
      ...
      const settings = [
        {
          ...
          value: "องค์การบริหารส่วนตำบลหัวนา",
          desc: "ชื่อหน่วยงาน",
        },
        ...
        { name: "allowed_ip", value: "", desc: "IP Address ที่อนุญาต" },
        {
          ...
          value: "",
          desc: "Token สำหรับ Telegram Bot",
        },
        {
          name: "telegram_groups",
          value: '[{"name":"กลุ่มหลัก","chat_id":"","active":true}]',
          desc: "กลุ่มรับการแจ้งเตือน Telegram",
        },
        {
          name: "notify_clock_in",
          value: "1",
          desc: "แจ้งเตือนเมื่อลงเวลาเข้า",
        },
        {
          name: "notify_clock_out",
          value: "1",
          desc: "แจ้งเตือนเมื่อลงเวลาออก",
        },
        {
          name: "admin_username",
          value: "admin",
          desc: "ชื่อผู้ใช้สำหรับแอดมิน",
        },
        {
          name: "admin_password",
          value: "admin123",
          desc: "รหัสผ่านสำหรับแอดมิน",
        },
...
app.post("/api/getemployee", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT emp_code, full_name FROM employees WHERE status = $1",
      ["active"]
    );
...
app.post("/api/clockin", async (req, res) => {
  ...
  try {
    ...
    const now = client_time
      ? adjustClientTime(client_time)
    ...
    let message =
      ...
      (lat && lon
        ? `📍 พิกัด: *${location}*\n` +
          `🗺 แผนที่: [ดูแผนที่](https://www.google.com/maps/place/${lat},${lon})`
        : "📍 พิกัด: *ไม่มีข้อมูล*");

    // ส่งการแจ้งเตือนถ้าตั้งค่าไว้
    if (
      notifySettingResult.rows.length > 0 &&
      notifySettingResult.rows[0].setting_value === "1"
    ) {
...
app.post("/api/clockout", async (req, res) => {
  ...
  try {
    const { employee, lat, lon, line_name, line_picture, client_time } =
      req.body;
    ...
    try {
      ...
      for (const group of groups) {
        if (group.active && group.chat_id) {
          try {
            console.log(
              `Sending message to ${group.name} (${group.chat_id}) via GSA`
            );
            ...
            console.log(
              `Message sent to ${group.name} via GSA successfully:`,
              response.data
            );
          } catch (error) {
            console.error(
              `Error sending message to ${group.name} via GSA:`,
              error.message
            );
...
app.post("/api/admin/test-gas", async (req, res) => {
  ...
  try {
    ...
    if (!useGasUrl) {
      ...
      if (
        gasUrlResult.rows.length === 0 ||
        !gasUrlResult.rows[0].setting_value
      ) {
        return res.json({
          success: false,
          message: "ไม่พบ URL ของ GSA กรุณาตั้งค่าก่อน",
        });
      }
    ...
    if (!useGasUrl.startsWith("https://")) {
      return res.json({
        success: false,
        message: "URL ของ GSA ต้องขึ้นต้นด้วย https://",
      });
    }
    ...
    if (tokenResult.rows.length === 0 || !tokenResult.rows[0].setting_value) {
      return res.json({
        success: false,
        message: "ไม่พบ Token ของ Telegram กรุณาตั้งค่าก่อน",
      });
    }
    ...
    if (groupsResult.rows.length === 0 || !groupsResult.rows[0].setting_value) {
      return res.json({
        success: false,
        message: "ไม่พบข้อมูลกลุ่ม Telegram กรุณาตั้งค่าก่อน",
      });
    }
    ...
    if (!activeGroup) {
      return res.json({
        success: false,
        message: "ไม่พบกลุ่ม Telegram ที่เปิดใช้งาน",
      });
    }
    ...
    if (parseInt(countResult.rows[0].count) === 0) {
      ...
      const settings = [
        ...
        // เพิ่ม setting สำหรับ GAS URL
        {
          ...
          desc: "URL ของ Google Apps Script Web App",
        },
        // เพิ่ม setting สำหรับเลือกใช้ GSA แทน Telegram API โดยตรง
        {
          name: "use_gas_for_telegram",
          value: "1",
          desc: "ใช้ Google Apps Script สำหรับส่งข้อความไป Telegram (1=ใช้, 0=ไม่ใช้)",
        },
      ];
      ...
      for (const setting of newSettings) {
        const checkResult = await pool.query(
          "SELECT setting_name FROM settings WHERE setting_name = $1",
          [setting.name]
        );
...
app.post("/api/admin/login", async (req, res) => {
  ...
  try {
    ...
    if (
      username === adminUserResult.rows[0].setting_value &&
...
app.get("/api/admin/time-logs", async (req, res) => {
  ...
  try {
    ...
    const formattedLogs = result.rows
      .filter((log) => log && log.clock_in)
      .map((log) => {
        // ปรับเวลาให้เป็นเวลาไทย
        const clockInDate = new Date(
          new Date(log.clock_in).getTime() + 7 * 60 * 60 * 1000
        );
        const clockOutDate = log.clock_out
          ? new Date(new Date(log.clock_out).getTime() + 7 * 60 * 60 * 1000)
          : null;
        ...
        return {
          ...
          clock_out_date: clockOutDate
            ? clockOutDate.toLocaleDateString("th-TH")
            : "",
          clock_out_time: clockOutDate
            ? clockOutDate.toLocaleTimeString("th-TH")
            : "",
          duration: clockOutDate
            ? calculateDuration(new Date(log.clock_in), new Date(log.clock_out))
            : "",
        };
      });
...
app.put("/api/admin/employees/:id", async (req, res) => {
  console.log(
    "API: admin/employees PUT - แก้ไขข้อมูลพนักงาน",
    req.params,
    req.body
  );
...
app.delete("/api/admin/employees/:id", async (req, res) => {
  ...
  try {
    ...
    console.log(
      "Permanently deleted employee with ID:",
      id,
      "(",
      employee.full_name,
      ")"
...
app.get("/api/admin/dashboard", async (req, res) => {
  ...
  try {
    ...
    const formattedLogs = recentLogsResult.rows
      .filter((log) => log && log.clock_in)
      .map((log) => {
        // ปรับเวลาให้เป็นเวลาไทย
        const clockInDate = new Date(
...
app.get("/api/admin/time-logs/:id", async (req, res) => {
  console.log(
    "API: admin/time-logs/:id - ดึงข้อมูลการลงเวลาเฉพาะรายการ",
    req.params
  );
...
app.put("/api/admin/time-logs/:id", async (req, res) => {
  console.log(
    "API: admin/time-logs/:id PUT - แก้ไขข้อมูลการลงเวลา",
    req.params,
    req.body
  );
  ...
  try {
    ...
    const checkResult = await pool.query(
      "SELECT id FROM time_logs WHERE id = $1",
      [id]
    );
...
app.post("/api/admin/time-logs", async (req, res) => {
  ...
  try {
    const { employee_id, clock_in, clock_out, note, skip_notification } =
      req.body;
    ...
    if (!skip_notification) {
      ...
      if (
        ...
        // สร้างข้อความสำหรับส่งแจ้งเตือน
        const clockInDate = new Date(
          new Date(clock_in).getTime() + 7 * 60 * 60 * 1000
        );
        const thaiFormatter = new Intl.DateTimeFormat("th-TH", {
          year: "numeric",
          month: "long",
      ...
      if (clock_out) {
        ...
        if (
          notifyOutSettingResult.rows.length > 0 &&
          notifyOutSettingResult.rows[0].setting_value === "1"
        ) {
          // สร้างข้อความสำหรับส่งแจ้งเตือน
          const clockOutDate = new Date(
            new Date(clock_out).getTime() + 7 * 60 * 60 * 1000
          );
          const thaiFormatter = new Intl.DateTimeFormat("th-TH", {
            year: "numeric",
            month: "long",
    ...
    res.json({
      success: true,
      message: "เพิ่มข้อมูลการลงเวลาเรียบร้อยแล้ว",
      id: newId,
    });
  } catch (error) {
...
app.delete("/api/admin/time-logs/:id", async (req, res) => {
  console.log(
    "API: admin/time-logs/:id DELETE - ลบข้อมูลการลงเวลา",
    req.params
  );
...
app.get("/api/getTimeOffset", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      return res.json({
        success: true,
        time_offset: result.rows[0].setting_value,
      });
    } else {
...
app.post("/api/admin/import-employees", async (req, res) => {
  console.log(
    "API: admin/import-employees - นำเข้ารายชื่อพนักงานจากไฟล์",
    req.body
  );
...
app.post("/api/admin/cleanup-time-logs", async (req, res) => {
  console.log(
    "API: admin/cleanup-time-logs - เคลียร์ข้อมูลประวัติการลงเวลา",
    req.body
  );

  try {
    const { date_before, employee_id, export_before_delete, cleanup_type } =
      req.body;

    if (!date_before && !cleanup_type) {
      return res.json({
        success: false,
        message: "กรุณาระบุข้อมูลสำหรับการลบ เช่น วันที่หรือประเภทการลบ",
      });
    }

    let query =
      "SELECT t.id, t.employee_id, e.emp_code, e.full_name, t.clock_in, t.clock_out, t.note, t.status FROM time_logs t JOIN employees e ON t.employee_id = e.id WHERE 1=1";
    const params = [];
    let paramIndex = 1;
    ...
    if (export_before_delete) {
      exportData = dataToDelete.rows.map((row) => {
        const clockInDate = new Date(
          new Date(row.clock_in).getTime() + 7 * 60 * 60 * 1000
        );
        const clockOutDate = row.clock_out
          ? new Date(new Date(row.clock_out).getTime() + 7 * 60 * 60 * 1000)
          : null;
...
app.post("/api/admin/cleanup-inactive-employees", async (req, res) => {
  console.log(
    "API: admin/cleanup-inactive-employees - ล้างข้อมูลพนักงานที่ไม่ได้ใช้งาน",
    req.body
  );
  ...
  try {
    ...
    if (inactiveEmployees.rows.length === 0) {
      return res.json({
        success: false,
        message: "ไม่พบพนักงานที่มีสถานะไม่ใช้งาน",
      });
    }
    ...
    try {
      ...
      console.log(
        `ลบพนักงานทั้งหมด ${deleteEmployeesResult.rowCount} คน และข้อมูลการลงเวลา ${deletedLogCount} รายการ`
      );
...
app.get("/api/admin/backup-database", async (req, res) => {
  ...
  try {
    ...
    const timeLogsResult = await pool.query(
      "SELECT * FROM time_logs ORDER BY id"
    );
    ...
    const backupData = {
      ...
      settings: settingsResult.rows.filter(
        (s) => s.setting_name !== "admin_password"
      ), // ไม่รวมรหัสผ่านในข้อมูลสำรอง
    };
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=time_tracker_backup_${
    ...
    res
      .status(500)
      .json({ success: false, message: "เกิดข้อผิดพลาด: " + error.message });
  }
});
  ...
  try {
    ...
    const formattedLogs = result.rows.map((log) => {
      ...
      return {
        ...
        วันที่ออกงาน: clockOutDate
          ? clockOutDate.toLocaleDateString("th-TH")
          : "",
        เวลาออกงาน: clockOutDate
          ? clockOutDate.toLocaleTimeString("th-TH")
          : "",
        หมายเหตุ: log.note || "",
        สถานะ: log.status,
        พิกัดเข้า:
          log.latitude_in && log.longitude_in
            ? `${log.latitude_in}, ${log.longitude_in}`
            : "",
        พิกัดออก:
          log.latitude_out && log.longitude_out
            ? `${log.latitude_out}, ${log.longitude_out}`
            : "",
      };
    });
